...
function CardGameCountdown()
{
	...
	return <h3 className="text-6xl min-[420px]:text-8xl font-bold text-center text-yellow-400 font-ds-digital">
...
function CardGameTitle({status})
{
	...
	에 다시 만나요!</h3>;
	if(status === Status.ALREADY) return <h3 className={`${commonStyle} text-neutral-200`}>이미 참여하셨습니다</h3>;
	return <div className="relative flex justify-center">
		<h3 className={`${commonStyle} graphic-gradient text-transparent bg-clip-text`}>카드를 뒤집어 주세요!</h3>
		<p className="text-neutral-300 text-center absolute -bottom-8">* 본 이벤트는 마감된 이벤트입니다. 이벤트의 열기를 느껴보세요!</p>
	</div>;
}
