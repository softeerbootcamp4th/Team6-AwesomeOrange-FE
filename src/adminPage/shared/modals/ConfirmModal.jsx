...
function ConfirmModal({title, description, onConfirm})
{
	...
	return <div className="w-96 h-auto min-h-56 p-4 flex flex-col justify-between bg-white rounded-md shadow">
		<div className="flex-grow flex flex-col gap-2">
			...
			<p className="text-body-s font-medium">{description}</p>
		</div>
		<div className="flex gap-4 justify-end">
			<Button styleType="ghost" className="px-6 py-2" onClick={close}>취소</Button>
			<Button styleType="filled" className="px-6 py-2" onClick={async ()=>{ await close(); onConfirm(); }}>확인</Button>
		</div>
	</div>
}
