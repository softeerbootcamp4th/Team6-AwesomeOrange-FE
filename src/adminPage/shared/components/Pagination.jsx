...
{
	return <button onClick={onClick} disabled={disabled} type="button"
...
function Pagination({currentPage, setPage: _setPage, maxPage, length=5})
{
	...
	return <div className="flex gap-4">
		<PaginationButton onClick={setPage(currentPage - 5)} disabled={currentPage - 2 < 1}>&lt;&lt;</PaginationButton>
		<PaginationButton onClick={setPage(currentPage - 1)} disabled={currentPage - 1 < 1}>&lt;</PaginationButton>
		{
			getPaginationItem(currentPage, maxPage, length).map( (i)=><PaginationButton key={i} onClick={setPage(i)} highlighted={i === currentPage}>{i}</PaginationButton> )
		}
		<PaginationButton onClick={setPage(currentPage + 1)} disabled={currentPage + 1 > maxPage}>&gt;</PaginationButton>
		<PaginationButton onClick={setPage(currentPage + 5)} disabled={currentPage + 2 > maxPage}>&gt;&gt;</PaginationButton>
	</div>
}
