...
function BatchTimeUpdater() {
  ...
  const [batchTimeState, batchTimeDispatch] = useReducer(
    fcfsBatchControlReducer,
    null,
    getDefaultState,
  );
  ...
  return (
    <div className="flex justify-center flex-wrap gap-8 gap-y-2">
      <BatchTimeUpdateInput
        caption="오픈시간 일괄 설정"
        type="start"
        state={batchTimeState}
        dispatch={batchTimeDispatch}
      />
      <BatchTimeUpdateInput
        caption="종료시간 일괄 설정"
        type="end"
        ...
        dispatch={batchTimeDispatch}
      />
      <Button disabled={startTime === null || endTime === null} onClick={autoFill}>
        자동 채우기
      </Button>
    </div>
  );
