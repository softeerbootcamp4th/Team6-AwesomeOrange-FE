...
{
  return date.valueOf() + minutes * MINUTES;
}
...
function verifyItems(map, {startTime, endTime, prevSnapshot = new Map()})
{
  ...
  {
    const verifiedValue = verifyItem(unstableItem, {startTime, endTime, prevSnapshot: prevSnapshot.get(key)});
...
class FcfsData {
  ...
  modify(key, data, { startTime, endTime } ) {
    ...
    const verified = verifyItem({...oldItem, ...data}, {startTime, endTime, prevSnapshot: oldItem});
