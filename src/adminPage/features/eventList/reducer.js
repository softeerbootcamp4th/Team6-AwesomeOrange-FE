...
export function searchReducer(state, action)
{
	switch(action.type)
	{
		...
		case "set_sort": return {...state, sort: {...state.sort, 
			[action.target]: (action.value === "asc" || action.value === "desc") ? action.value : "none" }
		};
...
export function searchStateToQuery(state)
{
	...
	const paramObj = {
		search: state.query,
		filter : Object.entries(state.filter).filter( ([, value])=>value ).map( ([key])=>key ).join(","),
		sort: Object.entries(state.sort).filter( ([, value])=>value !== "none" ).map( ([key,value])=>`${key}:${value}` ).join(","),
		page: state.page,
		size: 5
	};
