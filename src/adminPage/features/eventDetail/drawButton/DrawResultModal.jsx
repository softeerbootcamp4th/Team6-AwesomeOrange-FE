...
{
	return <Fragment key={`${ranking}-${name}-${phoneNumber}`}>
...
function DrawResultModal({eventId})
{
	...
	const { hullRef, mountMap, scrollTo } = useScrollControl();
	...
	const drawResultGroup = new GroupMap();
	for(let item of drawResultData) drawResultGroup.set(item.ranking, item);

	function mapResultGroup([key, subset])
	{
		return <div className={`${tableStyle} gap-y-2 font-regular`} key={key} ref={(ref)=>mountMap(ref, key)}>
	...
	return <div className="w-full h-full flex flex-col justify-center items-center gap-4">
		...
		<div className="flex flex-wrap gap-4">
			{Array.from( {length:maxGrade}, (_,i)=><button key={`grade-${i+1}`} onClick={()=>scrollTo(i+1)}>{i+1}등</button> )}
		</div>
