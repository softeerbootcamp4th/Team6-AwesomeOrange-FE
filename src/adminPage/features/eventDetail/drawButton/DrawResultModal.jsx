...
function DrawResultModal({eventId})
{
	...
	return <div className="w-full h-full flex flex-col justify-center items-center gap-4">
		...
		<div className="flex flex-wrap gap-4">
			{Array.from( {length:maxGrade}, (_,i)=><button className={`${i+1 === intersectState ? "text-blue-400 font-bold" : "text-neutral-400"}`} key={`grade-${i+1}`} onClick={()=>scrollTo(i+1)}>{i+1}등</button> )}
		</div>
