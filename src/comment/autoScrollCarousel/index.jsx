...
function AutoScrollCarousel({speed=1, gap=0, children})
{
	...
	return <div className="w-full h-full overflow-hidden" {...eventListener}>
		<div style={{"--gap": gap+"px", transform: `translateX(${position* -1}px)`}} 
		className="relative touch-none">
			<div className={`${flexStyle} -translate-x-[calc(100%+var(--gap,0px))]`}>{children}</div>
			<div className={flexStyle} ref={ref}>{children}</div>
			<div className={`${flexStyle} translate-x-[calc(100%+var(--gap,0px))]`}>{children}</div>
		</div>
	</div>
}
